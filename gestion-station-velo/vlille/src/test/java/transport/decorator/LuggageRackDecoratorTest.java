...
public class LuggageRackDecoratorTest {
    
    protected Bike bike; 
    protected Transport decoratedBike; 

    protected Trottinette trottinette; 
    protected Transport decoratedTrottinette; 

    @BeforeEach
    public void init(){
        Transport.idC = 0;
        bike = new Bike(10);
        decoratedBike = new LuggageRackDecorator(bike);
        
        trottinette = new Trottinette( 10);
        decoratedTrottinette = new LuggageRackDecorator(trottinette); 
    ...
    @Test 
    public void isAvailableOkAtCreation(){
        for(int i=0 ; i<12;i++){
            decoratedBike.addNbUtilisation();
            decoratedTrottinette.addNbUtilisation();
        }
    ...
    @Test 
    public void testLuggageRackDecoratorToString(){
        String testBikeString = "Bike " + bike.getId() + " with a luggage rack";
        String testTrottinetteString = "Trottinette " + trottinette.getId() + " with a luggage rack";
